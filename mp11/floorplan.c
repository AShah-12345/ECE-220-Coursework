...
/*The purpose of this program is to implement a floorplanner that is designed to layout a set of rectangular modules
and optimizes the packing area of a VLSI system design. The is_leaf_node function works by checking if the passed
node has any children (nodes to the left or right) and returning the appropriate value. The function is_internal_node checks if the passed
node has at least one child and returns the appropriate value. The function is_in_subtree checks whether a passed node is rooted
in the subtree of another passed node. The function rotate swaps height and width of a passed node to rotate the tree. The function
recut works by swapping vertical and horizontal cutline of a slicing tree. The function swap_module checks the cutline and module of a passed
node ptr and sets it to either a NULL module and cutline or a module and UNDEFINED cutline accordingly. The function swap_topology 
swaps the topology of two subtrees rooted at two given nodes of the slicing tree. The function postfix_traversal recursively calls itself to
perform postfix traversal on the given slicing tree. The function get_total_resource traverses through the tree and return the sum of all resource for each module.
Lastly, the function init_slicing_tree initializes a slicing tree by aligning modules along the horizontal line and produces a left-skewed tree.*/
...
int is_leaf_node(node_t* ptr) { //DONE
  // TODO: (remember to modify the return value appropriately)
  if(ptr->left == NULL && ptr->right == NULL) //check if the node has children
  {
...
int is_internal_node(node_t* ptr) { //DONE
  // TODO: (remember to modify the return value appropriately)
  if(ptr-> left != NULL || ptr->right != NULL) //check if node has at least one child
  {
    return 1; //if so, return
  }
  return 0; //return 0 if both children present
}

// Function: is_in_subtree
// Return 1 if the given subtree rooted at node 'b' resides in the subtree rooted at node 'a'.
int is_in_subtree(node_t* a, node_t* b) { //DONE
  // TODO: (remember to modify the return value appropriately)
  node_t *copy = b; //copy of b so we don't change its original value
  while(copy != NULL) //while there is a node to check
  {
    if(copy == a) //check if equal to a
    {
      return 1; //if so, return 1
    }
    else
    {
      copy = copy->parent; //iterate to the next parent value
    }
  }
...
void rotate(node_t* ptr) { //DONE
  // TODO: 
  int copy = ptr->module->w; //define copy of width as placeholder
  ptr->module->w = ptr->module->h; //swap width to height value
  ptr->module->h = copy; //swap height to the placeholder
}
...
void recut(node_t* ptr) { //DONE
  ...
  {
    ptr->cutline = H; //if so, change it to horizontal
  }
  else //if cutline is horizontal
  {
    ptr->cutline = V; //change it to vertical
  }
}

// Procedure: swap_module
// Swap the two modules between two given leave nodes in the slicing tree.
void swap_module(node_t* a, node_t* b) { //DONE
  ...
  // TODO:
  module_t *copy = a->module; //define a placeholder variable for a's module
  a->module = b->module; //copy a module to b module
  b->module = copy; //copy b module to a module (from our placeholder)
}
...
void swap_topology(node_t* a, node_t* b) { //DONE, check
  ...
  // TODO:
  if(a->parent->left == a) //check if a is to the left
  {
    a->parent->left = b; //if so, copy it to b
  }
  else //if it is to the right
  {
    a->parent->right = b; //copy right to b
  }
  if(b->parent->left == b) //check if b is to the left
  {
    b->parent->left = a; //if so, copy it to b
  }
  else //if it is to the right
  {
    b->parent->right = a; //copy right to a
  }
  node_t *copy = a->parent; //set a copy node to be a placeholder for a
  a->parent = b->parent; //set a parent to be the parent of b
  b->parent = copy; //set the parent b to be the parent of a via copy placeholder
}
...
void get_expression(node_t* root, int N, expression_unit_t* expression) { //DONE
  int i;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) { //UNDONE
  
  if(ptr == NULL) return; //this is our base case

  // TODO:
  postfix_traversal(ptr->left, nth, expression); //recursive call for ptr->left
  postfix_traversal(ptr->right, nth, expression); //recursive call for ptr->right

  if(ptr->module != NULL) //check if ptr->module is not NULL
  {
    expression[*nth].module = ptr->module; //set expression[nth] with module value
    expression[*nth].cutline = UNDEFINED_CUTLINE; //set expression[nth]'s cutline value with an undefined value
  }
  if(ptr->cutline != UNDEFINED_CUTLINE) //check if ptr->cutline is undefined
  {
    expression[*nth].module = NULL; //set expression[nth]'s module value with a NULL value
    expression[*nth].cutline = ptr->cutline; //set expression[nth] with cutline value
  }
  (*nth) += 1; //increment var to move to next nth value
}

// get_total_resource
// traverse through the tree and return the sum of all resource for each module
int get_total_resource(node_t* ptr) //DONE, check
{
  // TODO:
  int sum = 0; //set sum variable
  if(ptr == NULL) //check if null, base case
  {
    return 0; //return 0
  }
  else if(is_leaf_node(ptr) == 1) //check if ptr is a leaf node using predefined function
  {
    return ptr->module->resource; //return module resource
  }
  sum = sum + get_total_resource(ptr->left) + get_total_resource(ptr->right); //recursively call for the next left and right member, adding back to sum accordingly
  return sum; //return
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if(n==num_modules-1){ //here the counter has reached the end of the tree. You must return at the end of the loop
    ...
    //define params according to struct def above
    temp->left = NULL;
    temp->right = NULL; // no kids, (end of the road)
    temp->cutline=UNDEFINED_CUTLINE; //according to method instructions
    temp->parent=par;
    temp->module = &(modules[n]); //mem location of nth module
    return temp;
    
  }
  temp->module=NULL;
  temp->cutline=V; //not in base case so we begin the slice
  temp->parent=par;

  node_t * tempRight = (node_t *) malloc(sizeof(node_t)); //make space for leaf
  tempRight->cutline = UNDEFINED_CUTLINE; //according to method def
  tempRight->right = NULL;
  tempRight->left = NULL; //dead end of tree
  tempRight->parent = temp;
  tempRight->module = &modules[n]; //init module
  temp->right = tempRight;
  temp->left= init_slicing_tree(temp,n+1); //call next step in implementation
  //increment count by 1 since we traverse tree into next slice
